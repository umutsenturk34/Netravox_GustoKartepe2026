...
function HeroBlock({ data }) {
  ...
  const alignClass = textPosition === "bottom"
    ? "items-end"
    : "items-center";
  const paddingClass = textPosition === "bottom"
    ? "pb-16 pt-24"
    : "py-24";
  const gradientDir = textPosition === "bottom"
  ...
  return (
    <section className={`relative min-h-[92vh] flex ${alignClass} overflow-hidden`}>
      {data.image && (
        <Image
          src={data.image}
          alt={title || "Hero"}
      ...
      <div className={`relative z-10 container-shell ${paddingClass} max-w-2xl mr-auto`}>
        {eyebrow && (
...
function GalleryBlock({ data }) {
  ...
  if (layout === "featured" && images.length >= 2) {
    const [main, ...rest] = images.filter(Boolean);
    return (
        ...
        <div className="container-shell">
          ...
          <div className="grid gap-4 lg:grid-cols-[1.15fr_0.85fr]">
            <div className="group relative h-[480px] overflow-hidden rounded-2xl">
              <Image src={main} alt="Restoran atmosferi" fill className="object-cover transition duration-500 group-hover:scale-105" sizes="55vw" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              {rest.slice(0, 4).map((src, i) => (
                <div key={i} className="group relative overflow-hidden rounded-xl" style={{ height: "228px" }}>
                  <Image src={src} alt={`Görsel ${i + 2}`} fill className="object-cover transition duration-500 group-hover:scale-105" sizes="25vw" />
                </div>
              ))}
            </div>
          </div>
          {ctaText && ctaUrl && (
            <div className="mt-8 text-center md:hidden">
              <Link href={ctaUrl} className="inline-flex items-center gap-1.5 text-xs font-bold uppercase tracking-[0.2em]" style={{ color: "var(--bordeaux)" }}>
                {ctaText} <ArrowRight size={13} />
              </Link>
            </div>
          )}
...
function StatsBlock({ data }) {
  ...
  return (
    <section className="py-14 bg-white">
      <div className="container-shell">
        <div className={`grid gap-6 grid-cols-2 ${items.length >= 4 ? "md:grid-cols-4" : `md:grid-cols-${items.length}`}`}>
          {items.map((s, i) => (
            <div
              key={i}
    ...
    <section className="py-20 bg-white">
      ...
      >
        ...
        <div className="lg:flex-1 space-y-5">
          {eyebrow && (
            <p className="text-[10px] font-bold tracking-[0.35em] uppercase" style={{ color: "var(--bordeaux)" }}>
              {eyebrow}
            </p>
          )}
          {title && (
    ...
    <section className="py-16 bg-[var(--cream)]">
      <div className="container-shell max-w-3xl space-y-3">
        {items.map((it, i) => {
          ...
          return (
            <details key={i} className="group rounded-xl border bg-white overflow-hidden" style={{ borderColor: "var(--beige)" }}>
              <summary className="flex cursor-pointer items-center justify-between gap-4 px-6 py-4 text-sm font-semibold text-[var(--dark)] list-none select-none hover:bg-[var(--beige)]/30 transition">
                {q}
                <ArrowRight size={15} className="shrink-0 rotate-90 transition-transform group-open:rotate-[270deg]" style={{ color: "var(--bordeaux)" }} />
              </summary>
              <div className="px-6 pb-5 pt-1 text-sm leading-7 text-[var(--muted)]">{a}</div>
            </details>
          );
        })}
      </div>
          ...
          {items.map((it, i) => (
            ...
            >
              ...
              <div className="p-5">
                ...
                {it.description && (
                  <p className="mt-2 text-xs leading-relaxed text-[var(--muted)]">{it.description}</p>
                )}
              </div>
            </article>
          ))}
            ...
            return (
              <li key={i} className="py-7 flex gap-8 items-start">
                ...
                <div className="flex-1">
                  {vTitle && (
                    <p className="font-bold text-[var(--dark)] text-base" style={{ fontFamily: "var(--font-playfair)" }}>
                      {vTitle}
                    </p>
                  )}
                  {vDesc && (
                    <p className="mt-2 text-sm leading-relaxed text-[var(--muted)]">{vDesc}</p>
                  )}
                </div>
              </li>
            );
    ...
    <>
      {sorted.map((block, i) => {
        switch (block.type) {
          ...
          case "gallery":      return <GalleryBlock      key={i} data={block.data || {}} />;
          case "cta":          return <CtaBlock          key={i} data={block.data || {}} />;
          case "stats":        return <StatsBlock        key={i} data={block.data || {}} />;
          case "image-text":   return <ImageTextBlock    key={i} data={block.data || {}} />;
          case "faq":          return <FaqBlock          key={i} data={block.data || {}} />;
          case "testimonials": return <TestimonialsBlock key={i} data={block.data || {}} />;
          case "cards":        return <CardsBlock        key={i} data={block.data || {}} />;
          case "values":       return <ValuesBlock       key={i} data={block.data || {}} />;
          default:             return null;
        }
      })}
    </>
